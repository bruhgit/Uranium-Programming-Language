...
NativeValueKind parameterNativeKind(const std::string& annotation) {
    ...
    if (normalized.empty() || normalized == "Any" || normalized == "Number") {
        ...
        switch (instruction.op) {
            case FASTPATH_CONSTANT: {
                if (instruction.operand >= function->chunk.constants.values.size() ||
                    (!function->chunk.constants.values[instruction.operand].isNumber() &&
                     !function->chunk.constants.values[instruction.operand].isBool())) {
                    if (reason != nullptr) {
...
bool compileNativeJit(const FunctionPtr& function,
    ...
    for (std::size_t index = 0; index < plan.instructions.size(); ++index) {
        ...
        switch (instruction.op) {
            case FASTPATH_CONSTANT:
                if (function->chunk.constants.values[instruction.operand].isBool()) {
                    assembler.emitMovsdXmm0FromConstant(
                        function->chunk.constants.values[instruction.operand].asBool() ? 1.0 : 0.0);
                } else {
                    assembler.emitMovsdXmm0FromConstant(
                        function->chunk.constants.values[instruction.operand].asNumber());
                }
