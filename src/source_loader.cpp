...
bool isPropertyMemberReference(const std::string& text, std::size_t identifierStart) {
    if (identifierStart == 0) {
        return false;
    }

    std::size_t probe = identifierStart;
    while (probe > 0) {
        probe--;
        char current = text[probe];
...
std::string rewriteModuleSource(
    ...
    while (i < source.size()) {
        ...
        bool propertyMember = isPropertyMemberReference(source, start);
